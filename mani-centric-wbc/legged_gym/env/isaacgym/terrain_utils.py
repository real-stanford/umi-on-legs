...
def checker_height_map(
    terrain_size: Tuple[float, float],  # of entire terrain in meters
    grid_size: float,  # of a single grid cell in meters
    height_low: float,
    height_high: float,
):
    cells_per_col = int(np.ceil(terrain_size[0] / grid_size))
    cells_per_row = int(np.ceil(terrain_size[1] / grid_size))
    grid = np.ones((cells_per_col, cells_per_row), dtype=np.float32) * height_low

    col_mask = np.zeros((cells_per_col, cells_per_row), dtype=bool)
    col_arange = np.arange(cells_per_col // 2 + 1) * 2
    col_mask[col_arange[col_arange < cells_per_col], :] = True
    row_mask = np.zeros((cells_per_col, cells_per_row), dtype=bool)
    row_arange = np.arange(cells_per_row // 2 + 1) * 2
    row_mask[:, row_arange[row_arange < cells_per_row]] = True
    high_mask = np.logical_or(
        np.logical_and(col_mask, row_mask), np.logical_and(~col_mask, ~row_mask)
    )
    grid[high_mask] = height_high
    return grid


def generate_perlin_noise_2d(
    shape: Tuple[int, int], res: Tuple[int, int]
) -> np.ndarray:
    def f(t):
